...
def build_grad_custom(verts, edges, edge_tangent_vectors, vert_edge_outgoing,  rhs_matrix_list):
    edge_tangent_vectors = edge_tangent_vectors.numpy()
    ...
    data_vals = []
    eps_reg = 1e-5
    lhs_mat_all = [(edge_tangent_vectors[vert_edge_outgoing[i]].T @ edge_tangent_vectors[vert_edge_outgoing[i]])[np.newaxis] for i in range(N)]
    lhs_mat_all = np.concatenate(lhs_mat_all, axis=0) + eps_reg * np.identity(2)[np.newaxis]
    lhs_mat_all = np.linalg.inv(lhs_mat_all)
    lhs_mat_all = [lhs_mat_all[i] @ edge_tangent_vectors[vert_edge_outgoing[i]].T for i in range(N)]
    mat = [lhs_mat_all[i] @ rhs_matrix_list[i] for i in range(N)]
    sol_coefs = [(mat[i][0, :] + 1j * mat[i][1, :]).T for i in range(N)]
    data_vals = np.concatenate(np.array([sol_coefs[i] for i in range(N)]))
    # lhs_mid = [lhs_mat_all[i].T @ lhs_mat_all[i]]
    # row_inds = np.array(row_inds)
    # col_inds = np.array(col_inds)
    # data_vals = np.array(data_vals)
    # mat = scipy.sparse.coo_matrix(
    #     (data_vals, (row, col)), shape=(
    #         N, N)).tocsc()
...
def build_grad(verts, edges, edge_tangent_vectors):
    ...
    for iV in range(N):
        ...
        # assert ((sol_mat - mat[iV])**2).mean() < 1e-6
        sol_coefs = (sol_mat[0, :] + 1j * sol_mat[1, :]).T
        # assert ((sol_coefs - sol_coefs_all[iV])**2).mean() < 1e-6
        # assert np.linalg.norm((sol_coefs - data_vals_all[iV])) / len(sol_coefs) < 1e-4
    ...
    mat = scipy.sparse.coo_matrix(
        (data_vals, (row_inds, col_inds)), shape=(
            N, N)).tocsc()
    # assert (np.real(mat.to_coo().data) - data_vals_all
    return  mat
...
def compute_operators(verts, faces, k_eig, normals=None):
    ...
    if is_cloud:
        ...
        edges = torch.tensor(np.stack((inds_row, inds_col), axis=0), device=device, dtype=faces.dtype).long()
