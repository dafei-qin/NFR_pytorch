...
class LearnedTimeDiffusion(nn.Module):
    ...
    def forward(self, x, L, mass, evals, evecs):
        ...
        # self.diffusion_time.to(x.device)
        ...
        if self.method == 'spectral':
            ...
            # Diffuse
            time = self.diffusion_time.to(x.device)
            # print('t: ', time.device)
            # print('evals: ', evals.device)
    ...
    def forward(self, x_in, mass, L, evals, evecs, gradX, gradY):
        ...
        if self.with_gradient_features:
            ...
            for b in range(B):
                ...
                x_gradX = torch.mm(gradX, x_diffuse[b,...])
                x_gradY = torch.mm(gradY, x_diffuse[b,...])
