...
class DiffusionNetBlock(nn.Module):
    ...
    def forward(self, x_in, mass, L, evals, evecs, gradX, gradY):
        ...
        if self.with_gradient_features:
            ...
            # x_gradX = torch.mm(gradX, x_diffuse.transpose(0, 1).reshape(x_diffuse.shape[1], -1)).reshape(x_diffuse.shape[1], B, -1).transpose(0, 1)
            # x_gradY = torch.mm(gradY, x_diffuse.transpose(0, 1).reshape(x_diffuse.shape[1], -1)).reshape(x_diffuse.shape[1], B, -1).transpose(0, 1)
